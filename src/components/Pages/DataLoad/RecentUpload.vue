    ...
    <div class="w-full p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-8">
        ...
        <div class="flex items-center justify-between mb-4">
            <h5 class="text-xl font-bold leading-none text-gray-700">
                Transacciones Recientes <span class="text-sm font-normal text-gray-500">- Últimos 7 días</span>
            </h5>
        </div>
        ...
        <div>
            ...
            <div v-for="(transactionsByDate, date) in transactions" :key="date">
                <div v-if="activeTab === date" class="p-4">
                    <h3 class="text-lg font-semibold mb-4">Detalles del {{ formatDate(date) }}</h3>
                    <div class="flex flex-wrap gap-4 overflow-y-auto max-h-96">
                        <div v-for="transaction in transactionsByDate" :key="transaction.id"
                            ...
                            <p class="font-semibold">{{ formatDate(transaction.operationDate) }}</p>
...
const fetchData = async () => {
    try {
        ...
        if (response?.data) {
            ...
            // Ordenar las fechas en orden descendente y seleccionar la más reciente
            const sortedDates = Object.keys(transactions.value).sort((a, b) => new Date(b) - new Date(a));
            if (sortedDates.length > 0) {
                activeTab.value = sortedDates[0]; // Seleccionar la fecha más reciente
            }
        }
    } catch (error) {
