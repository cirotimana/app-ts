<template>
  <div class="flex justify-between items-center my-4">
    ...
    <div class="flex items-center space-x-4">
      ...
      <button class="bg-orange-500 px-3 py-2 text-white rounded-md hover:bg-orange-700"
        @click="downloadExcel">
        Descargar Excel
      </button>
    </div>
...
const initialColumns = [
  ...
  { label: "#Operacion", field: "operation_number", sortable: true, width: "10%" },
  { label: "Cliente", field: "beneficiary", sortable: true, width: "25%", columnClasses: ["text-orange-500"] },
...
const fetchAllFilteredData = async () => {
  try {
    ...
    while (allData.length < totalItems) {
      ...
      const response = await getBcps(params);
...
const filterRows = computed(() => {
  return tableConfig.rows.filter((x) => {
    ...
    const matchesValidated =
      validatedFilter.value === "" || x.status === (validatedFilter.value === "true");
